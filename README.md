...
2. **采集**：点「❤ 采集」抓取完整喜欢列表，顶部进度条实时显示进度；预检接口状态需几秒到几十秒，属正常
...
- 风控自愈：接口被限自动暂停并提示，稍后重新点采集时弹出滑块验证，完成后自动断点续采
...
- 取链自愈：取链失败自动刷新页面状态并重试；接口确实受限时明确提示稍后
...
- **统一响应分类器**：所有脚本用同一套分类判定「有效 JSON / 验证页 / 黑洞 / 假数据」，杜绝漏判
...
- **采集数量比抖音显示的喜欢数少？** 正常：已下架 / 删除 / 私密的内容接口不再返回，少几千到几万条为正常现象。
...
- **取链失败？** 应用会自动刷新页面状态重试；接口确实受限时会提示稍后重试。
...
在「选择」模式下勾选多个作品，点击「取消点赞」即可逐个取消抖音点赞。原有「删除」仍仅删除本地记录，不会改变。

- 复用应用现有的 WebView2 登录态，不保存或上传 Cookie
- 逐个处理并保留操作间隔，避免连续快速请求
- 仅在抖音接口确认成功后移除对应本地记录
- 失败项目保留在本地列表，并显示失败原因
